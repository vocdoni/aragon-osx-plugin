...
/**
 * Returns the managing DAO' vocdoni voting address defined in the environment variables. Throws if not found
 *
 * @export
 * @param {HardhatRuntimeEnvironment} hre
 * @return {string}
 */
export function getManagingDAOVocdoniVotingAddress(
  hre: HardhatRuntimeEnvironment
): string {
  const {network} = hre;
  const address =
    process.env[`${network.name.toUpperCase()}_MANAGINGDAO_VOCDONIVOTING`];
  if (!address) {
    throw new Error(
      `Failed to find managing DAO vocdoni voting address in env variables for ${network.name}`
    );
  }
  return address;
}
...
export const CURRENT_PROTOCOL_VERSION = [1, 3, 0];
